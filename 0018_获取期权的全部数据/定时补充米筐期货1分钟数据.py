...
for index, row in info_df.iterrows():
    ...
    try:
        ...
        if name + ".csv" not in file_list:
            df = rqdatac.get_price(name, start_date=begin_date, end_date=end_date, frequency='1m')
            if df is not None:
            ...
            try:
                ...
                d1 = pd.to_datetime(end_date, format='%Y-%m-%d').date()
            d2 = pd.to_datetime(last_time).date()
            # print(f"name = {name}, d1 = {d1}, d2 = {d2}")
            if d1 > d2:
                # print(pre_df)
                df = rqdatac.get_price(name, start_date=d2.strftime("%Y-%m-%d"), end_date=d1.strftime("%Y-%m-%d"), frequency='1m')
                df = df.reset_index()
